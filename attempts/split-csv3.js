...
// 7 slaves: 57 seconds, windows 7, core i3
// 5 slaves: 57 seconds, windows 7, core i3
...
var slaves = [];
slaves.push(spawn('node', ['csv2-3', '--port', 7000 + slaves.length]))
slaves.push(spawn('node', ['csv2-3', '--port', 7000 + slaves.length]))
slaves.push(spawn('node', ['csv2-3', '--port', 7000 + slaves.length]))
slaves.push(spawn('node', ['csv2-3', '--port', 7000 + slaves.length]))
slaves.push(spawn('node', ['csv2-3', '--port', 7000 + slaves.length]))
...
setTimeout(function () {
  ...
  var clients = [];
  clients.push( net.connect({port: 7000 + clients.length}) )
  clients.push( net.connect({port: 7000 + clients.length}) )
  clients.push( net.connect({port: 7000 + clients.length}) )
  clients.push( net.connect({port: 7000 + clients.length}) )
  clients.push( net.connect({port: 7000 + clients.length}) )
